document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderParticipantDetails(participant) {
        ...
        const isClaimed = participant.status === "Claimed";
        const foodBadgeClass = participant.food.toLowerCase().includes("non") ? "bg-warning text-dark" : "bg-success";

        participantCard.innerHTML = `
            <div class="participant-details w-100">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h3 class="mb-0 fw-bold">${escapeHtml(participant.name)}</h3>
                    <span class="badge ${foodBadgeClass} px-3 py-2 fs-6">${escapeHtml(participant.food)}</span>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <strong>${escapeHtml(participant.college)}</strong>
                </div>

                <div class="detail-row">
                    <span>Food Preference:</span>
                    <strong>${escapeHtml(participant.food)}</strong>
                </div>

                ${isClaimed ? `
                    <div class="detail-row">
                        <span>Distribution Time:</span>
                        <strong>${escapeHtml(participant.claimedAt || "Earlier")}</strong>
                    </div>
    ...
    function claimFood(participantId) {
        const participants = getParticipants();
        const target = participants.find(p => p.id.toUpperCase() === participantId.toUpperCase());

        if (!target) {
            alert("Participant record not found.");
            return;
        }

        if (target.status === "Claimed") {
            alert("Food has already been claimed for this participant.");
            renderParticipantDetails(target);
            return;
        }
        ...
        target.claimedAt = timeString;

        saveParticipants(participants);
        renderClaimSuccess(target);
    }
    ...
    function verifyQRCode(code) {
        ...
        const participant = findOrGenerateParticipant(qrValue);
        ...
        if (participant.status === "Claimed") {
            ...
            updateScannerStatus("✓ QR verified — Ready to claim food", "active");
        }
    }
